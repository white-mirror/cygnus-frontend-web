...
const apiBaseUrl = runtimeConfig.apiBaseUrl || process.env.VITE_API_BASE_URL || null;

function logEnvironment() {
  const relevantKeys = ['APP_VARIANT', 'VITE_API_BASE_URL', 'VITE_DEV_API_PROXY'];
  console.log('[electron] Variables de entorno activas:');
  for (const key of relevantKeys) {
    const value = process.env[key];
    console.log(`  - ${key}: ${value !== undefined ? value : '(sin definir)'}`);
  }
  console.log(`[electron] API base URL efectiva: ${apiBaseUrl ?? '(sin definir)'}`);
}

function createMainWindow() {
  if (!fs.existsSync(windowIconPath)) {
    console.warn(`[electron] Icono no encontrado en ${windowIconPath}. Se utilizará el valor por defecto del sistema.`);
  }
  ...
  if (isDev) {
    ...
    console.log(`[electron] Cargando aplicación desde servidor de desarrollo: ${devUrl}`);
...
app.whenReady().then(() => {
  ...
  console.log(`[electron] Iniciando Cygnus en variante "${appVariant}". Entorno empaquetado: ${app.isPackaged}`);
  logEnvironment();
  createMainWindow();
