...
4. Cada build genera un número incremental y coloca todos los artefactos dentro de `release/<variant>-<version>.<build>` (por ejemplo, `release/prod-0.0.1.42/`). El contador se guarda en `release/build-number.json` y empieza en 1; si necesitás reiniciarlo o alinearlo con CI podés tocar `BUILD_NUMBER_SEED` o fijar `BUILD_NUMBER` antes de ejecutar el script.
