...
4. Cada build genera un número incremental (almacenado en `package.json:buildNumber`) y coloca todos los artefactos dentro de `release/<variant>-<version>.<buildNumber>` (por ejemplo, `release/local-0.1.3.251111672/`). Si necesitás forzar un valor podés exportar `BUILD_NUMBER` antes de ejecutar el script o dejar que `build-number-generator` cree uno nuevo automáticamente.
